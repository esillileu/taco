...
1. `uv run taco plan intent template`
2. Agent decomposes user design into multiple intent payloads.
3. `uv run taco plan intent create-many --intents-json <INTENTS_JSON>`
4. `uv run taco plan intent list`
5. `uv run taco plan intent view --intent-id <INTENT_ID>`
6. `uv run taco plan intent propose --intent-id <INTENT_ID>`
7. `uv run taco plan intent autodesign --intent-id <INTENT_ID> --proposal-fingerprint <PROPOSAL_FP>`
8. `uv run taco plan intent generate-tasks --intent-id <INTENT_ID> --design-fingerprint <DESIGN_FP>`
9. `uv run taco plan intent review-bundle --intent-id <INTENT_ID> --proposal-fingerprint <PROPOSAL_FP> --design-fingerprint <DESIGN_FP> --taskset-fingerprint <TASKSET_FP> [--retry-on-fail 1]`
10. `uv run taco plan intent apply --intent-id <INTENT_ID> --decision-fingerprint <DECISION_FP> --approve --apply`
11. `uv run taco plan validate`
...
- Provide template and mapping rules for intent records.
- Let agents decompose user natural-language design into multiple intents.
- Convert decomposed intent payloads into explicit design intent records.
- Update intent/plan metadata and queue state through gated apply operations.
- Produce task blueprints with explicit design reasoning requirements.
