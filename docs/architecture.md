...
- 아키텍처 문서는 전체 구조를 설명하는 전역 SSOT로 유지한다.
- 태스크별 편의를 위해 아키텍처 내용을 분해/복제하지 않는다.
- 대신, 컨텍스트 추출 대상이 되는 섹션에 안정적인 참조 식별자를 부여한다.
- 태스크 문서는 필요한 식별자만 선언하고, pack은 해당 식별자만 조립한다.
...
- 문서 내 유일해야 한다.
- 의미를 반영하는 안정적인 이름을 사용한다.
- 순서 변경이나 헤딩 이동에도 유지한다.
...
- 문서 선택 로직도 `selector/group` 하드코딩 대신 `참조 식별자 해석기`로 분리한다.
